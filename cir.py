...
def circular_coefficients(phi, psi, R, c, m, n):
    ...
    def integrand_A_cos(theta, r):
        ...
        return phi(x,y) * mode_func_cos(r, theta) * r  # 包含雅可比行列式r
    ...
    def integrand_B_cos(theta, r):
        x, y = r*np.cos(theta), r*np.sin(theta)
        return psi(x,y) * mode_func_cos(r, theta) * r
    ...
    # 模式函数模板
    def mode_func_sin(r, theta, t=0):
        spatial = jn(m, alpha*r/R) * np.sin(m*theta)
        temporal = np.cos(omega*t) 
        return spatial * temporal
    ...
    def integrand_A_sin(theta, r):
        x, y = r*np.cos(theta), r*np.sin(theta)
        return phi(x,y) * mode_func_sin(r, theta) * r  # 包含雅可比行列式r
    ...
    def integrand_B_sin(theta, r):
        x, y = r*np.cos(theta), r*np.sin(theta)
        return psi(x,y) * mode_func_sin(r, theta) * r
...
def animate_circular_vibration(coeffs, R, duration, fps=24):
    ...
    for (m, n, A_cos, B_cos, A_sin, B_sin, omega) in coeffs:
        alpha = jn_zeros(m, n)[-1]
        spatial_cos = jn(m, alpha*R_grid/R) * np.cos(m*Theta_grid)
        spatial_sin = jn(m, alpha*R_grid/R) * np.sin(m*Theta_grid)
    ...
    def update(frame):
        ...
        for (A_cos, B_cos, A_sin, B_sin, omega, spatial_cos, spatial_sin) in spatial_modes:
            temporal_cos = A_cos*np.cos(omega*current_time) + B_cos*np.sin(omega*current_time)
            temporal_sin = A_sin*np.cos(omega*current_time) + B_sin*np.sin(omega*current_time)
            Z += temporal_cos * spatial_cos + temporal_sin * spatial_sin
